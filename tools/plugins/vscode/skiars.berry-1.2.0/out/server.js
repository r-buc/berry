...
var DECLARATION_KEYWORDS = /* @__PURE__ */ new Set(["def", "class", "var", "static"]);
var NAMESPACE_KEYWORDS = /* @__PURE__ */ new Set(["import", "as"]);
var CONSTANT_KEYWORDS = /* @__PURE__ */ new Set(["true", "false", "nil", "self", "super", "_class"]);
var ALL_KEYWORDS = /* @__PURE__ */ new Set([
  ...CONTROL_KEYWORDS,
...
var NUMBER_RE = /^(?:0[xX][A-Fa-f0-9]+|(?:\d+\.\d*|\.\d+|\d+)(?:[eE][+-]?\d+)?)/;
...
var BLOCK_OPEN = /* @__PURE__ */ new Set(["if", "while", "for", "do", "def", "class", "try"]);
function parseDocument(text) {
  ...
  const classStack = [];
  let i = 0;
  const peek = (offset) => tokens[i + offset];
  const cur = () => tokens[i];
  const skipPast = (check) => {
        ...
        while (i < tokens.length) {
          ...
          if (depth === 1 && t.kind === "identifier") {
            ...
            symbols.push({
              ...
              character: t.start,
              length: t.length,
              className: classStack.length > 0 ? classStack[classStack.length - 1] : void 0
            });
            i++;
            continue;
          }
  ...
  const unmatchedBlocks = blockStack.filter((e) => e.keyword === "class" || e.keyword === "def" || e.keyword === "if" || e.keyword === "while" || e.keyword === "for" || e.keyword === "do" || e.keyword === "try").map((e) => ({ keyword: e.keyword, line: e.line, character: e.character }));
  return { tokens, symbols, unmatchedBlocks, spuriousEnds };
}
function tokenAtPosition(tokens, line, character) {
  for (const tok of tokens) {
    if (tok.endLine !== void 0) {
...
function charBeforeWord(text, line, character) {
  const lines = text.split(/\r?\n/);
  const lineText = lines[line] ?? "";
  let start = character;
  while (start > 0 && IDENT_PART.test(lineText[start - 1])) start--;
...
var GLOBAL_FUNCTIONS = [
  {
    ...
    documentation: "Raises an `assert_failed` exception if `condition` is `false` or `nil`.\n\nAn optional `message` string is included in the exception.",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Prints all arguments to standard output, separated by spaces, followed by a newline.",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Reads a line from standard input and returns it as a string. If `prompt` is given it is written to stdout first (without a trailing newline).",
    parameters: [
  ...
  },
  {
    ...
    documentation: 'Returns a string describing the type of `value`.\n\nPossible return values: `"nil"`, `"bool"`, `"int"`, `"real"`, `"string"`, `"function"`, `"class"`, `"instance"`, `"module"`, `"list"`, `"map"`, `"range"`, `"comptr"`.',
    parameters: [
  ...
  },
  {
    ...
    documentation: "Converts `value` to a number. Strings are parsed; booleans become 0 or 1.",
    parameters: [
  ...
  },
  {
    name: "int",
    signature: "int(value)",
    documentation: "Converts `value` to an integer. Real numbers are truncated towards zero.",
    parameters: [
      { name: "value", description: "Value to convert." }
    ],
    returnType: "int"
  ...
  },
  {
    ...
    documentation: "Converts `value` to a boolean. `nil` and `false` are `false`; everything else is `true`.",
    parameters: [
      { name: "value", description: "Value to convert." }
    ],
    returnType: "bool"
  },
  {
    ...
    documentation: "Returns a super-instance view of `instance`, giving access to members of the superclass.",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Returns `true` if `object` is an instance of `class` or any subclass of it.",
    parameters: [
  ...
  },
  {
    ...
    documentation: 'Compiles the Berry source `code` string and returns a closure. `name` is an optional string used in error messages (defaults to `"(string)"`). Call the returned closure to execute the code.',
    parameters: [
  ...
  },
  {
    ...
    documentation: "Creates and returns a new empty module object, or returns the existing module with the given `name` if one already exists in the module registry.",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Returns the number of elements in `value`. Works for strings (byte count), lists, maps, and any object that implements `size()`.",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Returns a formatted string. The format string uses `printf`-style placeholders: `%d` (integer), `%f` (real), `%s` (string), `%g` (shorter of `%e`/`%f`), `%x` / `%X` (hex), `%o` (octal), `%b` (binary), `%%` (literal `%`).",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Returns an iterator for `value`. For objects that support it (list, map, range, custom classes with `iter()`), this returns a function that yields successive values.",
    parameters: [
...
var STRING_METHODS = [
  ...
  },
  {
    ...
    documentation: "Returns the index of the first occurrence of `sub`, or `-1` if not found. Search begins at `start` (default `0`).",
    parameters: [
  ...
  },
  {
    ...
    documentation: "Translates characters: each character in `from` is replaced by the corresponding character in `to`.",
    parameters: [
...
var MAP_METHODS = [
  ...
  },
  {
    ...
    documentation: "Returns the value associated with `key`, or `default` (nil if omitted) if the key is not present.",
    parameters: [
...
var JSON_FUNCTIONS = [
  {
    ...
    documentation: "Parses the JSON string `text` and returns the equivalent Berry value (map, list, string, number, bool, or nil).",
...
function buildFunctionMap(items) {
  return new Map(items.map((f) => [f.name, f]));
}
var GLOBAL_FUNCTION_MAP = buildFunctionMap(GLOBAL_FUNCTIONS);
var MATH_FUNCTION_MAP = buildFunctionMap(MATH_FUNCTIONS);
var JSON_FUNCTION_MAP = buildFunctionMap(JSON_FUNCTIONS);
var OS_FUNCTION_MAP = buildFunctionMap(OS_FUNCTIONS);
var DEBUG_FUNCTION_MAP = buildFunctionMap(DEBUG_FUNCTIONS);
var INTROSPECT_FUNCTION_MAP = buildFunctionMap(INTROSPECT_FUNCTIONS);
var STRING_METHOD_MAP = buildFunctionMap(STRING_METHODS);
var LIST_METHOD_MAP = buildFunctionMap(LIST_METHODS);
var MAP_METHOD_MAP = buildFunctionMap(MAP_METHODS);
var MODULE_NAMES = ["math", "json", "os", "debug", "introspect", "global", "solidify", "strict", "sys", "string", "path"];
var KEYWORDS = [
  "if",
  "elif",
  "else",
...
var TOKEN_TYPE_MAP = new Map(TOKEN_TYPES.map((t, i) => [t, i]));
var TOKEN_MOD_MAP = new Map(TOKEN_MODIFIERS.map((m, i) => [m, i]));
var SEMANTIC_LEGEND = { tokenTypes: TOKEN_TYPES, tokenModifiers: TOKEN_MODIFIERS };
var defaultSettings = {
  diagnostics: { enabled: true },
  trace: { server: "off" }
};
var hasConfigCapability = false;
  ...
  for (const spurious of parsed.spuriousEnds) {
    diagnostics.push({
      ...
      range: {
        ...
        end: { line: spurious.line, character: spurious.character + 3 }
        // 'end'.length === 3
      },
...
function buildSemanticTokens(doc, parsed) {
  ...
  const push = (line, startChar, length, type, mods = []) => {
    ...
    const modBits = mods.reduce((bits, m) => {
      ...
      return idx !== void 0 ? bits | 1 << idx : bits;
    }, 0);
  ...
  const pushMultiLine = (tok, type, mods = []) => {
    ...
    for (let l = tok.line; l <= endLine; l++) {
      const lineText = doc.getText(import_node.Range.create(l, 0, l, 1e4));
...
connection.onCompletion((params) => {
  ...
  for (const fn of GLOBAL_FUNCTIONS) {
    items.push({
      ...
      detail: fn.signature,
      documentation: { kind: import_node.MarkupKind.Markdown, value: fn.documentation }
    });
  }
  for (const mod of MODULE_NAMES) {
    items.push({ label: mod, kind: import_node.CompletionItemKind.Module });
  }
  for (const sym of parsed.symbols) {
    if (sym.name === word) continue;
    switch (sym.kind) {
      case "class":
        items.push({ label: sym.name, kind: import_node.CompletionItemKind.Class });
        break;
      case "function":
        items.push({
          label: sym.name,
      ...
      case "method":
        ...
        break;
      case "variable":
        items.push({ label: sym.name, kind: import_node.CompletionItemKind.Variable });
        break;
      case "parameter":
        items.push({ label: sym.name, kind: import_node.CompletionItemKind.Variable });
        break;
      case "import":
        items.push({ label: sym.name, kind: import_node.CompletionItemKind.Module });
        break;
    }
  }
...
function buildMemberCompletions() {
  ...
  const addMethods = (methods) => {
    for (const m of methods) {
      items.push({
        ...
        kind: import_node.CompletionItemKind.Method,
        detail: m.signature,
        documentation: { kind: import_node.MarkupKind.Markdown, value: m.documentation }
      });
    }
  };
  ...
  for (const [name, doc] of Object.entries(MATH_CONSTANTS)) {
    items.push({ label: name, kind: import_node.CompletionItemKind.Constant, documentation: doc });
  }
  return deduplicateCompletions(items);
}
...
connection.onHover((params) => {
  ...
  if (constDoc) {
    return { contents: { kind: import_node.MarkupKind.Markdown, value: `**${word}** \u2014 ${constDoc}` } };
  }
  ...
  if (MATH_CONSTANTS[word]) {
    return { contents: { kind: import_node.MarkupKind.Markdown, value: `**math.${word}** \u2014 ${MATH_CONSTANTS[word]}` } };
  }
  ...
  if (sym) {
    ...
    if (md) {
      return {
        contents: { kind: import_node.MarkupKind.Markdown, value: md },
        range: {
...
**Returns:** \`${fn.returnType}\``;
  ...
  return { contents: { kind: import_node.MarkupKind.Markdown, value: md } };
}
connection.onDefinition((params) => {
  ...
  const sym = matches[0];
  return import_node.Location.create(doc.uri, import_node.Range.create(
...
connection.onDocumentSymbol((params) => {
  ...
  for (const sym of parsed.symbols) {
    ...
    const range = import_node.Range.create(
      { line: sym.line, character: sym.character },
      { line: endLine, character: sym.character + sym.length }
    );
    const selRange = import_node.Range.create(
      { line: sym.line, character: sym.character },
      { line: sym.line, character: sym.character + sym.length }
    );
    let skind;
    switch (sym.kind) {
      case "class":
        skind = import_node.SymbolKind.Class;
        break;
      case "function":
        skind = import_node.SymbolKind.Function;
        break;
      case "method":
        skind = import_node.SymbolKind.Method;
        break;
      case "variable":
        skind = import_node.SymbolKind.Variable;
        break;
      case "import":
        skind = import_node.SymbolKind.Module;
        break;
      default:
        skind = import_node.SymbolKind.Variable;
        break;
    ...
    const ds = {
      ...
      children: sym.kind === "class" ? [] : void 0
    };
    if (sym.kind === "class") {
...
connection.onReferences((params) => {
  ...
  for (const t of parsed.tokens) {
    if (t.kind === "identifier" && t.text === word) {
      ...
      locs.push(import_node.Location.create(
        doc.uri,
        import_node.Range.create({ line: t.line, character: t.start }, { line: t.line, character: t.start + t.length })
      ));
    }
  }
  return locs;
...
connection.onSignatureHelp((params) => {
  ...
  const paramInfos = fn.parameters.map((p) => ({
    label: p.name,
    documentation: { kind: import_node.MarkupKind.Markdown, value: p.description + (p.optional ? " *(optional)*" : "") }
  }));
  const sig = {
    label: fn.signature,
    documentation: { kind: import_node.MarkupKind.Markdown, value: fn.documentation },
