    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        while (i < line.length) {
            ...
            if (IDENT_START.test(ch)) {
                let j = i + 1;
                while (j < line.length && IDENT_PART.test(line[j]))
                    j++;
                const word = line.slice(i, j);
                tokens.push({
                    kind: exports.ALL_KEYWORDS.has(word) ? 'keyword' : 'identifier',
                    text: word,
                    line: lineIdx,
                    start: i,
                    length: j - i,
...
function parseDocument(text) {
    ...
    const skipPast = (check) => {
        while (i < tokens.length && !check(tokens[i]))
            i++;
    };
    while (i < tokens.length) {
        ...
        if (word === 'var' || word === 'static') {
            ...
            while (i < tokens.length) {
                const t = cur();
                if (t.line !== declLine && t.kind !== 'operator')
                    break;
                ...
                if (t.kind === 'operator' && t.text === '=') {
                    ...
                    while (i < tokens.length) {
                        ...
                        if (u.line !== declLine)
                            break;
                        if (u.kind === 'operator' && (u.text === ',' || u.text === ';'))
                            break;
                        i++;
                    }
                    continue;
    ...
    for (const tok of tokens) {
        if (tok.endLine !== undefined) {
            ...
            if (line < tok.line || line > tok.endLine)
                continue;
            if (line === tok.line && character < tok.start)
                continue;
            if (line === tok.endLine && character >= (tok.endChar ?? 0))
                continue;
            return tok;
        }
        if (tok.line !== line)
            continue;
        if (character >= tok.start && character < tok.start + tok.length)
            return tok;
    }
    return undefined;
}
...
function wordAtPosition(text, line, character) {
    ...
    let start = character;
    while (start > 0 && IDENT_PART.test(lineText[start - 1]))
        start--;
    let end = character;
    while (end < lineText.length && IDENT_PART.test(lineText[end]))
        end++;
